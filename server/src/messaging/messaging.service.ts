...
export class MessagingService {
  ...
  async createChannel(createChannelDto: Omit<Channel, 'id'>): Promise<Channel> {
  ...
  async updateChannel(id: string, updateChannelDto: Partial<Channel>): Promise<Channel | null> {
  ...
  async getChannelMessages(channelId: string, options?: { limit?: number; before?: string }): Promise<TeamMessage[]> {
  ...
  async sendMessage(channelId: string, messageData: Omit<TeamMessage, 'id' | 'timestamp'>): Promise<TeamMessage> {
    const channel = await this.getChannel(channelId);
    if (!channel) {
      throw new NotFoundException('Channel not found');
    }
  ...
  async getUserDMs(userId: string): Promise<Channel[]> {
    return this.channels.filter(channel =>
      channel.type === 'DM' && channel.members.includes(userId)
    );
  }
  ...
  async getMessagingStats(): Promise<any> {
    ...
    const channelTypes = this.channels.reduce((acc, channel) => {
      acc[channel.type] = (acc[channel.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const messageTypes = this.messages.reduce((acc, message) => {
      acc[message.type] = (acc[message.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const avgMembersPerChannel = this.channels.reduce((sum, channel) => sum + channel.members.length, 0) / totalChannels;
  ...
  async getChannelActivity(channelId: string, days: number = 7): Promise<any> {
    ...
    const messagesByDay = channelMessages.reduce((acc, msg) => {
