...
export const ResponseTopology: React.FC<Props> = ({ nodes, selectedNodeId, onSelectNode, generatedPlan, playbooks, onSimulate, onExecute, onReset }) => {
  return (
                ...
                <div className="p-4 space-y-4 flex-1 lg:overflow-y-auto custom-scrollbar">
                    {!generatedPlan ? (
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                            ...
                            <div className="pt-4 flex gap-2"><Button onClick={onExecute} variant="primary" className="flex-1" disabled={generatedPlan.status !== 'DRAFT'}>{generatedPlan.status === 'EXECUTING' ? 'DEPLOYING...' : generatedPlan.status === 'COMPLETED' ? 'DEPLOYED' : 'EXECUTE PLAN'}</Button><Button onClick={onReset} variant="secondary">RESET</Button></div>
                        </div>
                    )}
                </div>
                </Card>
