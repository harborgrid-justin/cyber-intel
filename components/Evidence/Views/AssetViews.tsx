...
import { threatData } from '../../../services/dataLayer'; // Needed for updates
  ...
  <Card className="p-0 overflow-hidden h-full flex flex-col">
    ...
    <div className="flex-1 overflow-y-auto p-4">
        <ResponsiveTable<Device> data={devices} keyExtractor={d => d.id} columns={[
            ...
            { header: 'Recommendation', render: d => <span className="text-[10px] text-slate-400 font-bold uppercase">{ForensicsLogic.suggestDeviceAction(d)}</span> }
...
export const StorageOpsView: React.FC<{ artifacts: Artifact[] }> = ({ artifacts }) => {
  const metrics = ForensicsLogic.calculateStorageProjection(artifacts);
  return (
    <div className="space-y-6">
        <Grid cols={3}>
