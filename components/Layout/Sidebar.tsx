...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate }) => {
  ...
  return (
    <aside className="w-64 bg-slate-950 border-r border-slate-800 flex flex-col h-full shadow-2xl relative z-20" aria-label="Main Navigation">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-3 rounded-md text-sm font-medium transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-cyan-500 ${
                active 
                  ? 'bg-cyan-900/20 text-cyan-400 border border-cyan-800/50 shadow-[inset_0_0_10px_rgba(8,145,178,0.1)]' 
                  : 'text-slate-400 hover:text-white hover:bg-slate-900 border border-transparent'
              }`}
            >
              <Icon className={`w-4 h-4 ${active ? 'text-cyan-400' : 'text-slate-500 group-hover:text-white'}`} aria-hidden="true" />
              {item.label}
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-800" role="status">
