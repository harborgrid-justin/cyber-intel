...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, isOpen }) => {
  ...
  return (
    ...
    `}>
      ...
      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto px-3 py-6 space-y-8 custom-scrollbar" role="navigation">
      ...
      {/* User Profile Footer - Converted to Button for A11y */}
      <div className={`p-4 border-t border-[var(--colors-borderDefault)] shrink-0 bg-[var(--colors-surfaceDefault)]`}>
        <button 
          onClick={() => onNavigate(View.SETTINGS)} 
          className={`w-full flex items-center gap-3 p-2 rounded-lg hover:bg-[var(--colors-surfaceHighlight)] transition-colors group focus:outline-none focus:ring-2 focus:ring-[var(--colors-primary)]`}
