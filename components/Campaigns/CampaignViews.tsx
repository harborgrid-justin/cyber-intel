...
export const CampaignOverview: React.FC<{ campaign: Campaign }> = ({ campaign }) => (
  <div className="space-y-6">
    <Card className="p-6 border-l-4 border-l-cyan-500">
      <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Executive Summary</h3>
    ...
    <Grid cols={3}>
      ...
      <Card className="p-4">
        <h3 className="text-xs font-bold text-slate-500 uppercase mb-2">Timeline</h3>
        <div className="text-sm text-slate-300">{campaign.firstSeen} <span className="text-slate-500 mx-2">→</span> {campaign.lastSeen}</div>
      </Card>
      <Card className="p-4">
        <h3 className="text-xs font-bold text-slate-500 uppercase mb-2">Status</h3>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card className="p-4">
         <h3 className="text-xs font-bold text-slate-500 uppercase mb-3">Target Sectors</h3>
         <div className="flex flex-wrap gap-2">
            {campaign.targetSectors.map(s => <Badge key={s} color="blue">{s}</Badge>)}
         </div>
      </Card>
      <Card className="p-4">
         <h3 className="text-xs font-bold text-slate-500 uppercase mb-3">Geographic Reach</h3>
         <div className="flex flex-wrap gap-2">
            {campaign.targetRegions.map(r => <span key={r} className="px-3 py-1 bg-slate-800 border border-slate-700 rounded text-xs text-slate-300">{r}</span>)}
         </div>
      </Card>
    </div>
  </div>
...
export const CampaignIoCs: React.FC<{ threats: Threat[] }> = ({ threats }) => (
  <div className="space-y-4">
    <div className="flex justify-between items-center"><h3 className="text-white font-bold">Linked Indicators</h3><Badge color="red">{threats.length} IOCs</Badge></div>
...
export const CampaignAttribution: React.FC<{ actors: string[] }> = ({ actors }) => {
  ...
  return (
    <div className="space-y-4">
       <h3 className="text-white font-bold">Attributed Adversaries</h3>
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {actors.map(a => (
             <Card key={a} className="p-4 flex items-center gap-4 border-l-4 border-l-red-600 hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => handleProfile(a)}>
                <div className="w-10 h-10 rounded bg-slate-800 flex items-center justify-center text-slate-500 font-bold">{a.substring(0,2).toUpperCase()}</div>
