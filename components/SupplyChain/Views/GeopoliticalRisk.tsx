...
export const GeopoliticalRisk: React.FC<Props> = ({ vendors }) => {
  const highRiskVendors = vendors.filter(v => SupplyChainLogic.assessJurisdictionalRisk(v.hqLocation).score > 60);
  const safeVendors = vendors.filter(v => SupplyChainLogic.assessJurisdictionalRisk(v.hqLocation).score <= 30);
        ...
        <Card className="p-0 overflow-hidden relative">
            ...
            <div className="w-full h-full bg-slate-950 flex items-center justify-center p-8">
                ...
                <svg className="w-full h-full text-slate-800" viewBox="0 0 100 50">
                    ...
                    {vendors.map((v, i) => {
                        const isRisky = SupplyChainLogic.assessJurisdictionalRisk(v.hqLocation).score > 60;
            ...
            <Card className="p-0 overflow-hidden flex-1 border-red-900/30">
                ...
                <div className="p-4 space-y-3 overflow-y-auto">
                    {highRiskVendors.length > 0 ? highRiskVendors.map(v => {
                        const assessment = SupplyChainLogic.assessJurisdictionalRisk(v.hqLocation);
                        return (
                ...
                <div className="p-4 space-y-3 overflow-y-auto">
                    {safeVendors.map(v => (
                        <div key={v.id} className="flex justify-between items-center bg-green-900/10 border border-green-900/30 p-3 rounded">
                            ...
                            <div className="text-xl font-bold text-green-500">{SupplyChainLogic.assessJurisdictionalRisk(v.hqLocation).score}</div>
                        </div>
                    ))}
                </div>
            </Card>
