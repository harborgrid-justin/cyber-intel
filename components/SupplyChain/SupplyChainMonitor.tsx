...
import { Card, Badge, ProgressBar, FilterGroup, Grid, Button } from '../Shared/UI';
...
const SupplyChainMonitor: React.FC = () => {
  ...
  const riskData = useMemo(() => {
    ...
    // Using enriched mock vendors directly for this demo, linking to live threats
  ...
  return (
    <StandardPage title="Third-Party Risk Operations" subtitle="Supply Chain Risk Management (SCRM)" modules={[]} activeModule="" onModuleChange={() => {}}>
      <div className="flex flex-col gap-6 h-full">
        {/* Navigation Bar */}
        <div className="bg-slate-900 border-b border-slate-800 pb-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             <Card className="lg:col-span-2 p-0 overflow-hidden">
                <div className="p-4 bg-slate-950 border-b border-slate-800 flex justify-between items-center">
                   <h3 className="font-bold text-slate-400 uppercase tracking-widest text-sm">Critical Vendor Watchlist</h3>
        ...
        {activeModule === 'SBOM' && (
           <div className="flex h-full gap-6">
              ...
              <Card className="flex-1 p-0 overflow-hidden flex flex-col">
                 <div className="p-4 border-b border-slate-800 bg-slate-950 font-bold text-white text-sm">Software Bill of Materials: {selectedVendor.product}</div>
                    ...
                    <div className="space-y-2">
                       {v.access.map((acc, i) => (
                          <div key={i} className="bg-slate-950 p-2 rounded border border-slate-800 text-xs flex justify-between items-center">
                             ...
                             <span className="text-cyan-500 font-bold">{acc.accessLevel}</span>
                          </div>
                       ))}
                    </div>
                    <Button variant="outline" className="text-[10px] mt-2">REVOKE ACCESS</Button>
                 </Card>
              ))}
           </Grid>
        )}

        {/* --- MODULE 6: COMPLIANCE AUDITOR --- */}
        {activeModule === 'COMPLIANCE' && (
           <div className="space-y-4">
              {riskData.map(v => (
                 <div key={v.id} className="bg-slate-900 border border-slate-800 p-4 rounded flex items-center justify-between">
                    <div className="w-48 font-bold text-white">{v.name}</div>
                    <div className="flex-1 flex gap-4">
                       {v.compliance.map((c, i) => (
                          <div key={i} className={`flex items-center gap-2 px-3 py-1 rounded border text-xs ${c.status === 'VALID' ? 'bg-green-900/10 border-green-900 text-green-400' : c.status === 'EXPIRED' ? 'bg-red-900/10 border-red-900 text-red-400' : 'bg-yellow-900/10 border-yellow-900 text-yellow-400'}`}>
                             ...
                             <span className="opacity-50">exp: {c.expiry}</span>
                          </div>
                       ))}
        ...
        {/* --- MODULE 7: GEOPOLITICAL MAP --- */}
        {activeModule === 'GEO' && (
           <Card className="h-full p-6 relative overflow-hidden">
              <div className="absolute top-6 left-6 z-10">
                 <h3 className="font-bold text-white uppercase tracking-widest text-sm">Vendor Jurisdiction Map</h3>
              </div>
              {/* Reusing Abstract Map Visuals but focused on Vendor HQs */}
              <div className="w-full h-full flex items-center justify-center text-slate-600">
                 <div className="grid grid-cols-2 gap-12 text-center">
                    <div className="p-8 border-2 border-slate-800 rounded-xl">
                       <div className="text-4xl font-bold text-white mb-2">{riskData.filter(v => v.hqLocation === 'USA').length}</div>
                       <div className="text-xs font-bold text-green-500 uppercase">NATO / Friendly</div>
                    </div>
                    <div className="p-8 border-2 border-red-900/30 bg-red-900/5 rounded-xl">
                       <div className="text-4xl font-bold text-red-500 mb-2">{riskData.filter(v => ['Russia', 'China'].includes(v.hqLocation)).length}</div>
                       <div className="text-xs font-bold text-red-400 uppercase">Sanctioned / High Risk</div>
                    </div>
                 </div>
              </div>
           </Card>
