...
export const IncidentUsers = ({ cases }: { cases: Case[] }) => {
  ...
  // Mix in case assignees if they aren't in the system user list
       ...
       {pbs.map(pb => (
         <Card key={pb.id} className="p-4 flex flex-col justify-between hover:border-cyan-500 transition-colors">
            ...
            <div className="flex flex-wrap gap-1 mt-auto pt-2 border-t border-slate-800">
               {pb.tasks.map((t, i) => <span key={i} className="text-[10px] bg-slate-950 px-2 py-1 rounded text-slate-500 border border-slate-800">{t}</span>)}
            </div>
         </Card>
       ))}
     </div>
