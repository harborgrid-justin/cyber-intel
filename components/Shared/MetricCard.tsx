...
const MetricCardComponent: React.FC<MetricCardProps> = ({ 
  ...
  return (
    <Card 
      onClick={onClick} 
      className={`p-6 flex flex-col justify-between h-full relative overflow-hidden group ${onClick ? 'cursor-pointer hover:border-[var(--colors-borderHighlight)]' : ''} ${className}`}
    >
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-2">
            {IconComponent && (
                <div className={`p-1.5 rounded-md bg-[var(--colors-${colorClass}Dim)] border border-[var(--colors-${colorClass})] text-[var(--colors-${colorClass})]`}>
