    ...
    >
        ...
        <Grid cols={2}>
           <Card className="p-0 overflow-hidden">
             ...
             <div className="p-4 space-y-2">
               {['192.168.1.55', 'ceo@target.com'].map((q, i) => <div key={i} className="flex justify-between text-sm bg-slate-950 p-2 rounded border border-slate-800"><span className="text-slate-300 font-mono">{q}</span></div>)}
             </div>
           </Card>
           <Card className="p-0 overflow-hidden">
             <CardHeader title="Trending Topics" />
             <div className="p-4 flex flex-wrap gap-2">
               {['APT29', 'ZeroDay', 'Leak', 'Ransomware'].map(tag => <Badge key={tag} color="slate">{tag}</Badge>)}
             </div>
           </Card>
        </Grid>
      )}
      ...
      {activeModule === 'Metadata' && (
        <div className="space-y-6">
            ...
            <Card className="p-0 overflow-hidden">
                <CardHeader title="File Metadata Extraction" />
                <ResponsiveTable 
                    data={metaData} 
