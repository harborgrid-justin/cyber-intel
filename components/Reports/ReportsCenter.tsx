...
import { Card, Button, Badge, Grid, CardHeader, Input, FilterGroup, ProgressBar } from '../Shared/UI';
...
const ReportsCenter: React.FC<ReportsCenterProps> = ({ initialId }) => {
  ...
  const ReportList = () => (
    <div className="flex flex-col h-full bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 bg-slate-950 space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="font-bold text-white tracking-widest uppercase text-sm">Intelligence Products</h3>
                <Badge>{filteredReports.length}</Badge>
            </div>
            ...
            <Button onClick={() => setIsCreating(true)} className="w-full text-xs py-2">+ NEW REPORT</Button>
        </div>
            ...
            {filteredReports.map(r => (
                ...
                >
                    <div className="flex justify-between items-start mb-1">
                        <span className={`text-[10px] font-mono font-bold ${selectedReportId === r.id ? 'text-cyan-400' : 'text-slate-500 group-hover:text-slate-400'}`}>{r.id}</span>
                    ...
                    <div className="flex justify-between items-center text-[10px] text-slate-500">
                        <span className="flex items-center gap-1"><Icons.FileText className="w-3 h-3" /> {r.type}</span>
                        <span>{r.date}</span>
                    </div>
                </div>
            ))}
  ...
  const ReportsOverview = () => {
    ...
    return (
    <div className="flex-1 flex flex-col gap-6 h-full overflow-y-auto p-6">
       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-4 flex flex-col justify-between border-t-2 border-t-cyan-500">
             <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Total Assets</div>
          ...
          <Card className="p-4 flex flex-col justify-between border-t-2 border-t-green-500 bg-green-900/10">
             <div className="text-[10px] font-bold text-green-400 uppercase tracking-widest">Published</div>
          ...
          <Card className="p-4 flex flex-col justify-between border-t-2 border-t-yellow-500 bg-yellow-900/10">
             <div className="text-[10px] font-bold text-yellow-400 uppercase tracking-widest">Active Drafts</div>
          ...
          <Card className="p-4 flex flex-col justify-between border-t-2 border-t-slate-500">
             <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Archived</div>
                    ...
                    <div className="space-y-4">
                        ...
                        {drafts.slice(0, 3).map(d => (
                            <div key={d.id} onClick={() => setSelectedReportId(d.id)} className="flex items-center gap-4 p-4 bg-slate-900 border border-slate-800 rounded-lg hover:border-cyan-500 cursor-pointer group transition-all">
                                <div className="w-10 h-10 rounded bg-yellow-900/20 border border-yellow-500/30 flex items-center justify-center text-yellow-500">
                                    <Icons.FileText className="w-5 h-5" />
                                </div>
                                <div className="flex-1">
                                    <h4 className="font-bold text-white group-hover:text-cyan-400">{d.title}</h4>
                                    <div className="text-xs text-slate-500">Started by {d.author} • {d.date}</div>
                                </div>
  ...
  if (activeModule === 'Templates') {
    return (
      <StandardPage 
        ...
        modules={['Library', 'Templates', 'Scheduled']}
