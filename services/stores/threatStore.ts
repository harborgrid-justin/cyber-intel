...
export class ThreatStore extends BaseStore<Threat> {
  ...
  addThreat(threat: Threat, actors: any[], existingCases: any[], onAutoCase: (c: any) => void) {
    ...
    if (isDuplicate) {
      ...
      if (susp && !existingCases.find((c: any) => c.title.includes(susp))) {
        onAutoCase({ 
          ...
          relatedThreatIds: [], created: new Date().toISOString(), 
  ...
  updateStatus(id: string, status: IncidentStatus, existingCases: any[], onAutoCase: (c: any) => void) {
    const t = this.getById(id);
    if (t) {
      t.status = status;
      this.update(t);
      if (status === IncidentStatus.INVESTIGATING && !existingCases.find((c: any) => c.relatedThreatIds.includes(id))) {
        onAutoCase({ 
          ...
          relatedThreatIds: [t.id], created: new Date().toISOString(), 
          notes: [], artifacts: [], timeline: [], agency: 'SENTINEL_CORE', 
