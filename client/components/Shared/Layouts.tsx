...
export const StandardPage: React.FC<StandardPageProps> = ({ 
  ...
  return (
    <PageContainer noPadding className={className}>
      ...
      <div className={`flex-1 min-h-0 flex flex-col overflow-y-auto ${STYLES.page_padding}`}>
        {children}
      </div>
    </PageContainer>
  );
      ...
      <div className="flex justify-between items-start">
         <div className="flex items-center gap-3 overflow-hidden">
           ...
           <div className="min-w-0">
             <h2 className="text-lg md:text-xl font-bold text-white truncate leading-tight">{title}</h2>
             {subtitle && <p className="text-xs text-slate-500 font-mono truncate">{subtitle}</p>}
           </div>
         </div>
         {actions && <div className="flex gap-2 shrink-0 ml-2">{actions}</div>}
      </div>
