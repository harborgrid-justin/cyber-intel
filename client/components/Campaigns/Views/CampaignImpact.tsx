...
const CampaignImpact: React.FC<Props> = ({ campaign, compact = false }) => {
  ...
  const containmentPct = cases.length > 0 ? (cases.filter(c => c.status === 'CLOSED').length / cases.length) * 100 : 0;
      ...
      <div className={gridClass}>
         ...
         <Card className="p-6 bg-slate-900 border-l-4 border-l-blue-500">
            ...
            <div className={`${textScale} font-mono text-white font-bold`}>
                <AnimatedValue value={containmentPct} format="percent" />
            </div>
            <div className="text-[10px] text-slate-500 mt-2">{cases.length} Total Incident Cases</div>
            <div className="w-full bg-slate-800 h-1.5 mt-4 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 transition-all duration-1000 ease-out" style={{ width: `${containmentPct}%` }}></div>
            </div>
         </Card>
      </div>
