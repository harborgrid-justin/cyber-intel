          ...
          return (
            <div className={`bg-slate-900 border p-3 rounded shadow-sm group transition-colors ${
                ...
                <div className="mt-2 pt-2 border-t border-slate-800 flex justify-between items-center">
                    <div className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${sla.status === 'BREACHED' ? 'bg-red-900 text-red-300' : sla.status === 'WARNING' ? 'bg-orange-900 text-orange-300' : 'bg-slate-800 text-slate-400'}`}>
                        SLA: {sla.timeLeft}
                    </div>
                    <span className={`px-1.5 py-0.5 bg-slate-800 text-[9px] rounded font-bold ${t.score > 80 ? 'text-red-400' : 'text-slate-400'}`}>Score: {t.score}</span>
                </div>
            </div>
          );
        }}
