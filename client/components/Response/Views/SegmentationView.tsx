...
const MOCK_FLOWS: TrafficFlow[] = [
  ...
  { id: 'fl-4', source: '10.0.0.5 (WORKSTATION)', dest: '10.0.0.55 (ADMIN)', port: '445', allowed: true, timestamp: '10:05:00' }
];
...
export const SegmentationView: React.FC = () => {
  ...
  const handleSimulate = (policy: SegmentationPolicy) => {
    const result = OrchestratorLogic.simulatePolicy(policy, MOCK_FLOWS);
    setSimulationResult({
      id: policy.id,
  ...
  const handleCreate = () => {
      if(newPolicy.name && newPolicy.source && newPolicy.destination) {
          setPolicies([...policies, { ...newPolicy, id: `POL-${Date.now()}`, port: newPolicy.port || '*' } as SegmentationPolicy]);
          setIsCreating(false);
  ...
  const lateralPaths = useMemo(() => {
     return OrchestratorLogic.detectLateralPaths(threatData.getSystemNodes());
  }, []);
