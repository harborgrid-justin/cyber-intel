...
export const DeceptionOps: React.FC<Props> = ({ honeytokens }) => {
  ...
  const effectiveness = OrchestratorLogic.analyzeLureEffectiveness(honeytokens);
  const recommendations = OrchestratorLogic.recommendDecoyPlacement(threatData.getSystemNodes());

  const handleDeploy = () => {
      if(!newDecoy.name || !newDecoy.location) return;
      // In real app, this would call dataLayer.addHoneytoken
