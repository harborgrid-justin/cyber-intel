...
export const IdentityViews = {
  EmailBreach: () => {
    ...
    const [exposure, setExposure] = useState<{ score: number, types: string[] } | null>(null);

    const handleCheck = () => {
       setExposure(OsintLogic.checkBreachExposure(searchEmail, breaches));
    };
            ...
            <div className="flex-1 overflow-y-auto">
               <ResponsiveTable<OsintBreach> 
                 ...
                 columns={[
                    ...
                    { header: 'Entropy', render: b => {
                        const ent = OsintLogic.Identity.calculateCredentialEntropy(b.data === 'Hash' ? '8f2b...' : 'password123');
                        return <div className="w-24"><ProgressBar value={ent} color={ent < 50 ? 'red' : 'green'} /></div>
                    }}
                 ]}
  ...
  SocialGraph: () => {
    const profiles = threatData.getOsintSocial();
    
    return (
      <Grid cols={3}>
         {profiles.map(p => {
            const botProb = OsintLogic.detectBotProbability(p);
            const influence = OsintLogic.Identity.analyzeSocialInfluence(p);
                  ...
                  <div className="space-y-3">
                     <div>
                        <div className="flex justify-between text-xs mb-1">
                           ...
                           <span className={`font-bold ${botProb > 50 ? 'text-red-500' : 'text-green-500'}`}>{botProb}%</span>
                        </div>
                        <ProgressBar value={botProb} color={botProb > 50 ? 'red' : 'green'} />
                     </div>
                     <div className="flex justify-between items-center text-xs bg-slate-900 p-2 rounded">
                        ...
                        <span className="text-cyan-400 font-bold">{influence.label} ({influence.score})</span>
                     </div>
                  </div>
