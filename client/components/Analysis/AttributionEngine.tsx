...
const AttributionEngine: React.FC = () => {
  ...
  const [results, setResults] = useState<ReturnType<typeof ThreatLogic.calculateAttribution>>([]);
  ...
  const handleAnalyze = () => {
    ...
    setTimeout(() => {
      ...
      const matches = ThreatLogic.calculateAttribution(input, actors);
      setResults(matches);
      setAnalyzing(false);
    }, 800);
  };
        ...
        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar p-6">
          {analyzing ? (
             <div className="h-full flex items-center justify-center flex-col gap-4">
               ...
               <div className="text-xs text-cyan-500 font-mono animate-pulse">MATCHING TTPs AGAINST {threatData.getActors().length} PROFILES...</div>
             </div>
          ) : results.length > 0 ? (
            results.map((res, idx) => (
