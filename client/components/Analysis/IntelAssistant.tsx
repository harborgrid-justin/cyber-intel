  ...
  const handleSend = async (text: string) => {
    if (!chatSession.current) {
      setMessages(prev => [...prev, { 
        ...
        role: 'system', 
        text: "AI service unavailable. Please configure OPENAI_API_KEY.", 
        timestamp: Date.now() 
      }]);
      return;
    }
    ...
    try {
      const responseText = await chatSession.current.sendMessage(text);
      setMessages(prev => [...prev, { 
        id: (Date.now() + 1).toString(), 
        role: 'model', 
        text: responseText || "No Data", 
        timestamp: Date.now(), 
      ...
      {activeModule === 'Chat' && (
        <Card className="flex-1 flex flex-col min-h-0 p-0 border-slate-800 h-full overflow-hidden">
          <CardHeader 
            ...
            action={<Badge color="purple" className="text-[10px]">GPT-4O-MINI</Badge>} 
          />
          <ChatInterface 
            messages={messages}
            onSend={handleSend}
