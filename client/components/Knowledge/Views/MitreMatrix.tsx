...
export const MitreMatrix: React.FC<Props> = ({ matrixMode, setMatrixMode, onView }) => {
  ...
  // HEATMAP LOGIC: Calculate active hits per technique based on live threats
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex justify-between items-center mb-4 bg-slate-900 p-2 rounded border border-slate-800 shrink-0">
          ...
          <FilterGroup 
            value={matrixMode} 
            onChange={(v: any) => setMatrixMode(v)} 
            options={[
              {label:'Standard', value:'STANDARD'}, 
              {label:'Live Heatmap', value:'HEATMAP', color: 'bg-red-500'}
            ]} 
          />
      </div>
      
      <div className="overflow-x-auto pb-4 custom-scrollbar flex-1 bg-slate-950 border border-slate-800 rounded-xl p-4">
          ...
          <div key={tactic.id} className="flex-1 min-w-[180px] flex flex-col gap-2">
            ...
            <div className="space-y-2 overflow-y-auto custom-scrollbar pr-1 pb-2">
              {techniques.filter(t => t.tactic === tactic.name).map(tech => {
                ...
                if (hasActivity) {
                  cardClass = hits > 2 
                    ? 'bg-red-900/80 border-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)] z-10' 
                ...
                return (
                  <div 
                    key={tech.id} 
                    onClick={() => onView(tech.id, 'Technique')} 
                    className={`${cardClass} border p-3 text-[10px] cursor-pointer transition-all duration-200 rounded relative group shadow-sm`}
                  >
